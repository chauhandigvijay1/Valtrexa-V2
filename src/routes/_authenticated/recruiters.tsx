  ...
  const discoverMutation = useMutation({
    mutationFn: async () => {
      return apiPost<{ recruiters: any[]; companyName: string }>("/api/recruiters/discover", {
        companyName: discoveryCompany,
        roleTitle: discoveryRole || undefined,
      });
    },
                ...
                >
                  ...
                  <TableCell onClick={(e) => e.stopPropagation()}>
                    <div className="flex gap-2 text-muted-foreground">
                      ...
                      {r.phone && (
                        <a href={`tel:${r.phone}`} title={r.phone}>
                  ...
                  <TableCell onClick={(e) => e.stopPropagation()}>
                    <div className="flex gap-1">
                      ...
                      <Button
                        size="icon"
                        variant="ghost"
                        onClick={() => confirm("Delete recruiter?") && del.mutate(r.id)}
                      >
                              ...
                              <div>
                                ...
                                <span className="font-medium">
                                  {r.relevance_score !== null
                                    ? `${Math.round(Number(r.relevance_score) * 100)}%`
                                    : "—"}
                                </span>
                              </div>
                              <div>
            ...
            <div className="space-y-3">
              <div className="space-y-1.5">
                ...
                <Input
                  ...
                  onChange={(e) => setDiscoveryCompany(e.target.value)}
              ...
              <p className="text-xs text-muted-foreground">
                AI will identify likely recruiters, hiring managers, and engineering leads at the
                specified company.
              </p>
            </div>
            <DialogFooter>
              <Button variant="ghost" onClick={() => setDiscoveryOpen(false)}>
                Cancel
              </Button>
              <Button
                disabled={discoverMutation.isPending || !discoveryCompany.trim()}
                onClick={() => discoverMutation.mutate()}
              >
                {discoverMutation.isPending ? (
                  <>
                    <Loader2 className="h-4 w-4 animate-spin mr-2" /> Discovering…
                  </>
                ) : (
                  <>
      ...
      {editing && (
        <Dialog open onOpenChange={(o) => !o && setEditing(null)}>
          <DialogContent className="max-w-xl">
            <DialogHeader>
              <DialogTitle>{editing.id ? "Edit recruiter" : "New recruiter"}</DialogTitle>
            </DialogHeader>
            <div className="space-y-3">
              <div className="grid md:grid-cols-2 gap-3">
                <Field
                  label="Name *"
                  v={editing.name}
                  on={(v) => setEditing({ ...editing, name: v })}
                />
                <Field
                  label="Company"
                  v={editing.company}
                  on={(v) => setEditing({ ...editing, company: v })}
                />
                <Field
                  label="Email"
                  v={editing.email}
                  on={(v) => setEditing({ ...editing, email: v })}
                />
                <Field
                  label="Phone"
                  v={editing.phone}
                  on={(v) => setEditing({ ...editing, phone: v })}
                />
                <Field
                  label="LinkedIn"
                  v={editing.linkedin_url}
                  on={(v) => setEditing({ ...editing, linkedin_url: v })}
                />
                <Field
                  label="Profile URL"
                  v={editing.profile_url}
                  on={(v) => setEditing({ ...editing, profile_url: v })}
                />
                <div className="space-y-1.5">
                  <Label>Classification</Label>
                  <Select
                    value={editing.role ?? ""}
                    onValueChange={(val) => setEditing({ ...editing, role: val || null })}
                  >
                    <SelectTrigger>
                ...
                <div className="space-y-1.5">
                  ...
                  <Input
                    type="date"
                    value={editing.last_contacted_at?.slice(0, 10) ?? ""}
                    onChange={(e) =>
                      setEditing({
                        ...editing,
