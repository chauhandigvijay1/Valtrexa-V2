...
import { Bookmark, BookmarkCheck, Download, ExternalLink, Pencil, Radar, Sparkles, Trash2 } from "lucide-react";
...
export const Route = createFileRoute("/_authenticated/opportunities")({ component: OpportunitiesPage });
...
function OpportunitiesPage() {
  ...
  const primaryResumeId = resumes.data?.find((item) => item.is_primary)?.id ?? resumes.data?.[0]?.id ?? null;
  const activeImportConfig =
    importIntegrations.data?.find((item) => item.provider === sourceType)?.config as Record<string, string> | undefined;
  ...
  const importMutation = useMutation({
    mutationFn: async () => {
      ...
      if (!resolvedValue) throw new Error("Enter a board token, site, board URL, or search URL, or save one in Settings.");
      const source =
        sourceType === "greenhouse"
  ...
  return (
    <div>
      <PageHeader title="Opportunity Radar" description="Import jobs from real sources and score them against your primary resume." />
      <CrudShell
        search={search}
        onSearch={(value) => {
          setPage(1);
        ...
        filters={
          <>
            <Select value={statusFilter} onValueChange={(value) => { setPage(1); setStatusFilter(value as JobStatus | "all"); }}>
            ...
            <Select value={priorityFilter} onValueChange={(value) => { setPage(1); setPriorityFilter(value as JobPriority | "all"); }}>
              <SelectTrigger className="w-[140px]"><SelectValue /></SelectTrigger>
              <SelectContent>
            ...
            <Button variant={savedOnly ? "default" : "outline"} size="sm" onClick={() => { setPage(1); setSavedOnly(!savedOnly); }}>
              ...
              <TableRow key={job.id}>
                <TableCell className="font-medium">
                  <div className="flex items-center gap-2">
                    {job.title}
                    {job.url && <a href={job.url} target="_blank" rel="noreferrer" className="text-muted-foreground hover:text-foreground"><ExternalLink className="h-3 w-3" /></a>}
                  </div>
                </TableCell>
                ...
                <TableCell>
                  <div className="flex items-center gap-2">
                    <div className="w-16 h-1.5 bg-muted rounded-full overflow-hidden">
                      <div className="h-full bg-primary" style={{ width: `${Math.min(100, job.match_score ?? 0)}%` }} />
                    </div>
                    <span className="text-xs text-muted-foreground">{job.match_score ?? 0}%</span>
                  </div>
                </TableCell>
                <TableCell><Badge variant="outline" className="capitalize">{job.priority}</Badge></TableCell>
                <TableCell><Badge variant="secondary" className="capitalize">{job.status}</Badge></TableCell>
                <TableCell>
                  <div className="flex gap-1">
                    <Button size="icon" variant="ghost" aria-label="Generate match" onClick={() => matchMutation.mutate(job.id)}>
                      <Sparkles className="h-4 w-4" />
                    </Button>
                    <Button size="icon" variant="ghost" aria-label="Research company" disabled={!job.company_name?.trim()} onClick={() => openResearch(job)}>
                      <Radar className="h-4 w-4" />
                    </Button>
                    <Button size="icon" variant="ghost" aria-label={job.saved ? "Unsave job" : "Save job"} onClick={() => toggleSaved.mutate({ id: job.id, saved: !job.saved })}>
                      {job.saved ? <BookmarkCheck className="h-4 w-4 text-primary" /> : <Bookmark className="h-4 w-4" />}
                    </Button>
                    <Button size="icon" variant="ghost" aria-label="Edit job" onClick={() => setEditing(job)}><Pencil className="h-4 w-4" /></Button>
                    <Button size="icon" variant="ghost" aria-label="Delete job" onClick={() => confirm("Delete job?") && del.mutate(job.id)}><Trash2 className="h-4 w-4 text-destructive" /></Button>
                  </div>
                </TableCell>
              </TableRow>
            ))}
      ...
      {importing && (
        <Dialog open onOpenChange={(open) => !open && setImporting(false)}>
          <DialogContent className="max-w-xl">
            <DialogHeader><DialogTitle>Import jobs</DialogTitle></DialogHeader>
            <div className="space-y-3">
              ...
              <div className="space-y-1.5">
                ...
                <Input
                  ...
                  placeholder={savedSourceValue ? `Saved default: ${savedSourceValue}` : "Enter a source value or save one in Settings"}
                />
              </div>
              <div className="text-sm text-muted-foreground">
                Greenhouse, Lever, and Ashby use direct board ingestion. LinkedIn, Naukri, and Wellfound use real public-page scraping from the URL you provide or save in Settings.
              </div>
            </div>
            <DialogFooter>
              <Button variant="ghost" onClick={() => setImporting(false)}>Cancel</Button>
              <Button disabled={importMutation.isPending || (!sourceValue.trim() && !savedSourceValue)} onClick={() => importMutation.mutate()}>
          ...
          <DialogContent className="max-w-xl">
            ...
            <div className="space-y-3">
              <div className="grid md:grid-cols-2 gap-3">
                ...
                <Field label="Company" value={editing.company_name ?? ""} onChange={(value) => setEditing({ ...editing, company_name: value })} />
                <Field label="Location" value={editing.location ?? ""} onChange={(value) => setEditing({ ...editing, location: value })} />
                <Field label="Salary range" value={editing.salary_range ?? ""} onChange={(value) => setEditing({ ...editing, salary_range: value })} />
                <Field label="Source" value={editing.source ?? ""} onChange={(value) => setEditing({ ...editing, source: value })} />
                <Field label="URL" value={editing.url ?? ""} onChange={(value) => setEditing({ ...editing, url: value })} />
                <div className="space-y-1.5">
                  <Label>Status</Label>
                  <Select value={editing.status ?? "open"} onValueChange={(value) => setEditing({ ...editing, status: value as JobStatus })}>
                    <SelectTrigger><SelectValue /></SelectTrigger>
                ...
                <div className="space-y-1.5">
                  ...
                  <Select value={editing.priority ?? "medium"} onValueChange={(value) => setEditing({ ...editing, priority: value as JobPriority })}>
                    <SelectTrigger><SelectValue /></SelectTrigger>
                ...
                <div className="space-y-1.5">
                  ...
                  <Input type="number" min={0} max={100} value={editing.match_score ?? 0} onChange={(event) => setEditing({ ...editing, match_score: Math.max(0, Math.min(100, Number(event.target.value) || 0)) })} />
                </div>
              </div>
              <div className="space-y-1.5">
                <Label>Description</Label>
                <Textarea aria-label="Description" rows={6} value={editing.description ?? ""} onChange={(event) => setEditing({ ...editing, description: event.target.value })} />
              </div>
            </div>
            <DialogFooter>
              <Button variant="ghost" onClick={() => setEditing(null)}>Cancel</Button>
              <Button disabled={save.isPending || !editing.title} onClick={() => save.mutate(editing, { onSuccess: () => setEditing(null) })}>
