...
type Interview = {
  id: string; company_name: string; role_title: string | null; round: string | null;
  interviewer: string | null; scheduled_at: string | null; status: IvStatus;
  meeting_url: string | null; notes: string | null; application_id: string | null;
};
...
function InterviewsPage() {
  ...
  const q = useCrudList<Interview>({
    table: "interviews", searchColumn: "company_name", search: debounced, page,
    orderBy: "scheduled_at", ascending: false,
  ...
  return (
    <div>
      ...
      <CrudShell
        ...
        onNew={() => setEditing({ status: "scheduled" })} newLabel="New interview"
        filters={
      ...
      >
        <Table>
          <TableHeader><TableRow>
            ...
            <TableHead>Round</TableHead><TableHead>When</TableHead>
            <TableHead>Status</TableHead><TableHead>Link</TableHead>
          ...
          <TableBody>
            {rows.map((r) => (
              <TableRow key={r.id}>
                ...
                <TableCell><Badge variant="outline" className={`capitalize ${color[r.status]}`}>{r.status}</Badge></TableCell>
                <TableCell>{r.meeting_url && <a href={r.meeting_url} target="_blank" rel="noreferrer" className="text-muted-foreground hover:text-foreground"><ExternalLink className="h-4 w-4" /></a>}</TableCell>
                <TableCell>
                  <div className="flex gap-1">
                    ...
                    <Button size="icon" variant="ghost" onClick={() => confirm("Delete interview?") && del.mutate(r.id)}>
      ...
      {editing && (
        <Dialog open onOpenChange={(o) => !o && setEditing(null)}>
          <DialogContent className="max-w-xl">
            <DialogHeader><DialogTitle>{editing.id ? "Edit interview" : "New interview"}</DialogTitle></DialogHeader>
            <div className="space-y-3">
              <div className="grid md:grid-cols-2 gap-3">
                ...
                <div className="space-y-1.5"><Label>Round</Label>
                  <Input value={editing.round ?? ""} onChange={(e) => setEditing({ ...editing, round: e.target.value })} placeholder="Tech screen, onsite…" />
                </div>
                <div className="space-y-1.5"><Label>Interviewer</Label>
                  <Input value={editing.interviewer ?? ""} onChange={(e) => setEditing({ ...editing, interviewer: e.target.value })} />
                </div>
                <div className="space-y-1.5"><Label>Scheduled at</Label>
                  <Input type="datetime-local" value={editing.scheduled_at ? toLocal(editing.scheduled_at) : ""}
                    onChange={(e) => setEditing({ ...editing, scheduled_at: e.target.value ? new Date(e.target.value).toISOString() : null })} />
                </div>
                <div className="space-y-1.5"><Label>Status</Label>
                  <Select value={editing.status ?? "scheduled"} onValueChange={(v) => setEditing({ ...editing, status: v as IvStatus })}>
              ...
              <div className="space-y-1.5"><Label>Meeting URL</Label>
                <Input value={editing.meeting_url ?? ""} onChange={(e) => setEditing({ ...editing, meeting_url: e.target.value })} />
              </div>
              <div className="space-y-1.5"><Label>Notes / Outcome</Label>
                <Textarea rows={4} value={editing.notes ?? ""} onChange={(e) => setEditing({ ...editing, notes: e.target.value })} />
              </div>
            </div>
            <DialogFooter>
              <Button variant="ghost" onClick={() => setEditing(null)}>Cancel</Button>
              <Button disabled={save.isPending || !editing.company_name}
