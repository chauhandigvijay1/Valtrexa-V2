...
import { Pencil, Sparkles, Copy, RefreshCcw, Video, Calendar, Trash2, Plus, ShieldAlert } from "lucide-react";
...
function OutreachPage() {
  ...
  const campaigns = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase.from("outreach_campaigns").select("*").order("updated_at", { ascending: false });
  ...
  const messages = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase.from("outreach_messages").select("*").order("created_at", { ascending: false });
      if (error) throw error;
  ...
  const loomScripts = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase.from("loom_scripts" as any).select("*").order("created_at", { ascending: false });
      if (error) throw error;
  ...
  const research = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase.from("company_research").select("company_name,summary").order("updated_at", { ascending: false });
      if (error) throw error;
  ...
  const resumes = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase.from("resumes").select("id,title,is_primary").order("updated_at", { ascending: false });
      if (error) throw error;
  ...
  const recruiters = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase.from("recruiters").select("id,name,company").order("updated_at", { ascending: false });
      if (error) throw error;
      return (data ?? []) as RecruiterOption[];
    },
  });

  const defaultResumeId = resumes.data?.find((item) => item.is_primary)?.id ?? resumes.data?.[0]?.id ?? "";
  ...
  const followupsQuery = useQuery({
    queryKey: ["followups"],
    queryFn: async () => {
      const { data, error } = await supabase.from("followups").select("*").order("due_at", { ascending: true });
      if (error) throw error;
      return data ?? [];
    },
  });
  ...
  const applicationsQuery = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase.from("applications").select("id,company_name,role_title");
      if (error) throw error;
      return data ?? [];
    },
  });
  ...
  const toggleFollowupMutation = useMutation({
    mutationFn: async (payload: { id: string; done: boolean }) => {
      const { error } = await supabase.from("followups").update({ done: payload.done }).eq("id", payload.id);
      if (error) throw error;
    },
    onSuccess: () => {
  ...
  const createFollowupMutation = useMutation({
    mutationFn: async (payload: { applicationId: string | null; recruiterId: string | null; dueAt: string; note: string }) => {
  ...
  const campaignCards = useMemo(() => {
    ...
    const researchByCompany = new Map((research.data ?? []).map((item) => [item.company_name, item]));

    return (campaigns.data ?? [])
      .map((campaign) => {
        const template = parseCampaignTemplate(campaign.template);
        const companyName = typeof template.companyName === "string" && template.companyName.trim() ? template.companyName : campaign.name.replace(/\s+campaign$/i, "");
        const draftIds = template.drafts && typeof template.drafts === "object" ? (template.drafts as Record<string, string>) : {};
        const coldEmail = typeof draftIds.cold_email === "string" ? messagesById.get(draftIds.cold_email) ?? null : null;
        const linkedinMessage = typeof draftIds.linkedin_message === "string" ? messagesById.get(draftIds.linkedin_message) ?? null : null;
        const loomScriptId = typeof template.loomScriptId === "string" ? template.loomScriptId : null;
        const loomScript =
          (loomScriptId ? (loomScripts.data ?? []).find((item) => item.id === loomScriptId) : null) ??
          (loomScripts.data ?? []).find((item) => item.company_name === companyName) ??
          null;
        const campaignPainPoints = Array.isArray(template.painPointIds)
          ? template.painPointIds.map((id) => painPointsById.get(String(id))).filter((item): item is PainPointRow => Boolean(item))
      ...
      .filter((item) => {
        const haystack = [item.companyName, item.summary, item.coldEmail?.subject, item.linkedinMessage?.subject].filter(Boolean).join(" ").toLowerCase();
  ...
  return (
    <div className="space-y-6">
      <PageHeader title="Outreach Campaign Studio" description="Generate email copy, DM copy, and Loom assets from the same company research, pain points, recruiter context, and primary resume." />
      ...
      <Card className="p-6 space-y-4 border border-purple-500/10 bg-purple-500/5">
        <div className="flex items-center justify-between">
          ...
          <Button size="sm" variant="outline" className="border-purple-500/20 hover:bg-purple-500/10 text-purple-300" onClick={() => setAddFollowupOpen(true)}>
        ...
        <div className="grid gap-3 max-h-72 overflow-y-auto pr-2">
              ...
              >
                <div className="flex items-start gap-3">
                  <input 
                    ...
                    checked={fu.done} 
                    onChange={(e) => toggleFollowupMutation.mutate({ id: fu.id, done: e.target.checked })} 
                  />
                  <div>
                    <div className={`font-medium ${fu.done ? "line-through text-muted-foreground" : ""}`}>
                    ...
                    <div className="text-xs text-muted-foreground mt-0.5 flex items-center gap-2">
                      <span>Due: {new Date(fu.due_at).toLocaleDateString()} at {new Date(fu.due_at).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</span>
                      {isOverdue && <Badge variant="destructive" className="text-[9px] px-1 py-0 h-4">OVERDUE</Badge>}
                    </div>
                  </div>
                </div>
                <Button size="icon" variant="ghost" className="hover:bg-destructive/15 text-muted-foreground hover:text-destructive" onClick={() => deleteFollowupMutation.mutate(fu.id)}>
          ...
          {(followupsQuery.data ?? []).length === 0 && (
            <div className="text-center py-8 text-muted-foreground text-sm border border-dashed rounded-lg border-purple-500/20 bg-purple-500/5">
              No follow-ups scheduled yet. They are automatically generated when you submit applications.
            </div>
          )}
        </div>
      </Card>
      ...
      <Card className="space-y-4 p-4 md:p-6">
        <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
          <div className="relative flex-1 max-w-md">
            <Input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Search companies or drafts…" />
          </div>
      ...
      <div className="space-y-4">
        {campaignCards.map((card) => {
          const loomMetadata = (card.loomScript?.metadata ?? {}) as Record<string, string | string[]>;
          return (
              ...
              <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
                <div className="space-y-2">
                  <div className="flex flex-wrap items-center gap-2">
                    ...
                    <Badge variant="secondary">{card.painPoints.length} pain point{card.painPoints.length === 1 ? "" : "s"}</Badge>
                    {card.loomScript && <Badge variant="outline">Loom ready</Badge>}
                  </div>
                  <div className="text-sm text-muted-foreground">{card.summary ?? card.campaign.description ?? "No company summary stored for this campaign yet."}</div>
                  <div className="flex flex-wrap gap-2">
                    {card.painPoints.map((point) => (
                      <Badge key={point.id} variant={point.severity >= 4 ? "destructive" : "secondary"}>
                        {point.title}
                      </Badge>
                    ))}
                  </div>
                  ...
                  <Card className="space-y-3 p-4">
                    <div className="flex items-center justify-between gap-3">
                      <div>
                        ...
                        <div className="text-xs text-muted-foreground">Visible workflow output from pain point → talking points → script.</div>
                      </div>
                      {card.loomScript && (
                        <Button variant="outline" size="sm" onClick={() => copyText(card.loomScript?.script_text ?? "")}>
                    ...
                    {card.loomScript ? (
                      <div className="grid gap-3 lg:grid-cols-2">
                        ...
                        <InfoBlock title="Problem statement" value={String(loomMetadata.problem_statement ?? "")} />
                        <InfoBlock title="Solution pitch" value={String(loomMetadata.solution_pitch ?? "")} />
                        <InfoBlock title="CTA" value={String(loomMetadata.cta ?? "")} />
                        <div className="space-y-2 lg:col-span-2">
                          <div className="text-xs font-medium uppercase tracking-wide text-muted-foreground">Full script</div>
        ...
        {!campaigns.isLoading && !campaignCards.length && (
          <Card className="p-8 text-center text-muted-foreground">No campaigns yet. Generate one from stored company research and pain points.</Card>
        )}
      </div>
      ...
      {generateOpen && (
        <Dialog open onOpenChange={(open) => !open && setGenerateOpen(false)}>
          <DialogContent className="max-w-lg">
            <DialogHeader><DialogTitle>Generate outreach campaign</DialogTitle></DialogHeader>
            <div className="space-y-3">
              <div className="space-y-1.5">
                <Label>Company name *</Label>
                <Input aria-label="Company name *" list="company-options" value={draft.companyName} onChange={(event) => setDraft({ ...draft, companyName: event.target.value })} />
              ...
              <div className="space-y-1.5">
                ...
                >
                  ...
                  {(recruiters.data ?? [])
                    .filter((recruiter) => !draft.companyName || recruiter.company?.toLowerCase() === draft.companyName.toLowerCase())
                    .map((recruiter) => (
                      <option key={recruiter.id} value={recruiter.id}>{recruiter.name}{recruiter.company ? ` · ${recruiter.company}` : ""}</option>
                    ))}
                </select>
              </div>
              {!isSelectedCompanyHighTarget && (
                <div className="flex items-center gap-2 rounded-lg bg-amber-500/10 p-3 text-sm text-amber-500 border border-amber-500/20">
                  ...
                  <span>Strategic campaigns are gated to High Value Targets. Mark this company as a High Value Target in Company Research to unlock.</span>
                </div>
              )}
            </div>
            <DialogFooter>
              ...
              <Button
                disabled={generateCampaignMutation.isPending || !draft.companyName.trim() || !draft.resumeId || !isSelectedCompanyHighTarget}
                onClick={() => generateCampaignMutation.mutate({
      ...
      {editing && (
        <Dialog open onOpenChange={(open) => !open && setEditing(null)}>
          <DialogContent className="max-w-2xl">
            <DialogHeader><DialogTitle>Edit outreach message</DialogTitle></DialogHeader>
            <div className="space-y-3">
              <div className="space-y-1.5"><Label>Subject</Label><Input value={editing.subject ?? ""} onChange={(event) => setEditing({ ...editing, subject: event.target.value })} /></div>
              <div className="space-y-1.5"><Label>Body</Label><Textarea rows={12} value={editing.body ?? ""} onChange={(event) => setEditing({ ...editing, body: event.target.value })} /></div>
              <div className="space-y-1.5">
                <Label>Status</Label>
                <select
                  className="w-full rounded-md border border-input bg-background px-3 py-2 text-sm"
                  value={editing.status}
                  onChange={(event) => setEditing({ ...editing, status: event.target.value as OutreachMessageRow["status"] })}
                >
            ...
            <DialogFooter>
              ...
              <Button disabled={updateMutation.isPending} onClick={() => updateMutation.mutate(editing)}>
      ...
      {addFollowupOpen && (
        <Dialog open onOpenChange={(o) => !o && setAddFollowupOpen(false)}>
          <DialogContent className="max-w-md">
            ...
            <div className="space-y-4 py-2">
              ...
              <div className="space-y-1.5">
                ...
                <Select
                  ...
                  onValueChange={(val) => setNewFollowup({ ...newFollowup, applicationId: val === "none" ? "" : val })}
                >
                  <SelectTrigger>
            ...
            <DialogFooter>
              <Button variant="ghost" onClick={() => setAddFollowupOpen(false)}>Cancel</Button>
              <Button 
                onClick={() => {
                  createFollowupMutation.mutate({
                    applicationId: newFollowup.applicationId || null,
                    recruiterId: null,
                    dueAt: newFollowup.dueAt ? new Date(newFollowup.dueAt).toISOString() : new Date().toISOString(),
                    note: newFollowup.note,
                  });
                }}
                disabled={createFollowupMutation.isPending || !newFollowup.note || !newFollowup.dueAt}
              >
    ...
    <Card className="space-y-3 p-4">
      <div className="flex items-center justify-between gap-3">
        <div>
          ...
          <div className="text-xs text-muted-foreground">Generated automatically from the active campaign context.</div>
        </div>
        {message && (
          <div className="flex gap-2">
            <Button variant="outline" size="sm" onClick={() => copyText([message.subject, message.body].filter(Boolean).join("\n\n"))}>
              <Copy className="mr-2 h-4 w-4" />
              Copy
            </Button>
      ...
      {message ? (
        <div className="space-y-3">
          ...
          <div className="space-y-2">
            <div className="text-xs font-medium uppercase tracking-wide text-muted-foreground">Body</div>
...
function InfoBlock({ title, value }: { title: string; value: string }) {
  return (
    <div className="rounded-lg border border-border p-3">
      ...
      <div className="mt-2 text-sm text-foreground whitespace-pre-wrap">{value || "Not stored yet."}</div>
    </div>
  );
}
