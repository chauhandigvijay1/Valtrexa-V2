...
type Skill = { id: string; name: string; category: string | null; level: "beginner" | "intermediate" | "advanced" | "expert" };
      ...
      >
        <Table>
          <TableHeader><TableRow>
            ...
            <TableHead>Level</TableHead><TableHead className="w-24" />
              ...
              <TableRow key={r.id}>
                ...
                <TableCell>
                  <div className="flex gap-1">
                    ...
                    <Button size="icon" variant="ghost" onClick={() => confirm("Delete skill?") && del.mutate(r.id)}>
...
function SkillForm({ draft, onChange }: { draft: Partial<Skill>; onChange: (s: Partial<Skill>) => void }) {
  return (
    <div className="space-y-3">
      ...
      <div className="space-y-1.5"><Label>Category</Label>
        <Input value={draft.category ?? ""} onChange={(e) => onChange({ ...draft, category: e.target.value })} placeholder="Frontend, DevOps…" />
      </div>
      <div className="space-y-1.5"><Label>Level</Label>
        <Select value={draft.level ?? "intermediate"} onValueChange={(v) => onChange({ ...draft, level: v as Skill["level"] })}>
