...
import { Pencil, Trash2, Wand2, Loader2, Search, CalendarPlus, ChevronDown, ChevronUp, MessageSquare } from "lucide-react";
import { apiPost, apiGet } from "@/lib/api-client";
import { toast } from "sonner";

type AppStatus = "saved" | "applied" | "screening" | "interview" | "offer" | "rejected" | "withdrawn" | "accepted";
const STATUSES: AppStatus[] = ["saved", "applied", "screening", "interview", "offer", "rejected", "withdrawn", "accepted"];

type Application = {
  id: string; company_name: string; role_title: string; status: AppStatus;
  source: string | null; notes: string | null; applied_at: string | null;
  resume_version_id: string | null; job_id: string | null;
  tier?: string | null;
...
const statusColor: Record<AppStatus, string> = {
  saved: "bg-muted text-foreground", applied: "bg-blue-500/15 text-blue-400",
  screening: "bg-amber-500/15 text-amber-400", interview: "bg-purple-500/15 text-purple-400",
  offer: "bg-emerald-500/15 text-emerald-400", accepted: "bg-emerald-500/20 text-emerald-300",
  rejected: "bg-destructive/15 text-destructive", withdrawn: "bg-muted text-muted-foreground",
};
...
function ApplicationsPage() {
  ...
  const q = useCrudList<Application>({
    table: "applications", searchColumn: "company_name",
    search: debounced, page,
    extraFilter: (qb) => statusFilter !== "all" ? qb.eq("status", statusFilter) : qb,
    orderBy: "applied_at", ascending: false,
  });

  const resumes = useQuery({
    queryKey: ["resume-versions-list"], enabled: !!user,
    queryFn: async () => {
  ...
  const followupsQuery = useQuery({
    queryKey: ["followups-for-apps"], enabled: !!user,
    queryFn: async () => {
      const { data, error } = await supabase
  ...
  const generatePackageMutation = useMutation({
    mutationFn: async (r: Application) => {
      ...
      return apiPost<{ tier: string; qa: { question: string; answer: string }[]; storedAnswers?: any[] }>(
        "/api/applications/generate-package",
        { applicationId: r.id, companyName: r.company_name, jobId: r.job_id }
      );
    },
    onSuccess: async (data, r) => {
      await qc.invalidateQueries({ queryKey: ["applications"] });
      await qc.invalidateQueries({ queryKey: ["followups-for-apps"] });
      toast.success(`Application package generated — Tier ${data.tier} assigned. ${data.qa?.length ?? 0} Q&A generated.`);
      setExpandedId(r.id);
    },
  ...
  const createFollowUpMutation = useMutation({
    mutationFn: async (r: Application) => {
      return apiPost("/api/follow-ups/auto-create", {
        ...
        action: r.status === "applied" ? "applied" : r.status === "screening" ? "screening" : r.status === "interview" ? "interview" : "applied",
      });
    },
    onSuccess: async () => {
      ...
      >
        <Table>
          <TableHeader><TableRow>
            <TableHead>Company</TableHead><TableHead>Role</TableHead>
            <TableHead>Status</TableHead><TableHead>Tier</TableHead>
          ...
          <TableBody>
            {rows.map((r) => {
              const appFollowups = followups.filter((f: any) => f.application_id === r.id && !f.done);
              return (
                <>
                  <TableRow key={r.id}>
                    <TableCell className="font-medium">{r.company_name}</TableCell>
                    <TableCell>{r.role_title}</TableCell>
                    <TableCell><Badge className={`capitalize ${statusColor[r.status]}`} variant="outline">{r.status}</Badge></TableCell>
                    <TableCell>
                      {r.tier ? (
                    ...
                    <TableCell>
                      {appFollowups.length > 0 ? (
                        <Badge variant="secondary" className="text-xs">{appFollowups.length} pending</Badge>
                      ) : (
                        <span className="text-xs text-muted-foreground">—</span>
                      )}
                    </TableCell>
                    <TableCell className="text-sm text-muted-foreground">{r.applied_at ? new Date(r.applied_at).toLocaleDateString() : "—"}</TableCell>
                    <TableCell>
                      <div className="flex gap-1">
                        ...
                        <Button size="icon" variant="ghost" title="Create follow-up"
                        ...
                        <Button size="icon" variant="ghost" onClick={() => confirm("Delete application?") && del.mutate(r.id)}>
        ...
        <Dialog open onOpenChange={(o) => !o && setEditing(null)}>
          <DialogContent className="max-w-xl">
            ...
            <div className="space-y-3">
              <div className="grid md:grid-cols-2 gap-3">
                ...
                <div className="space-y-1.5"><Label>Role *</Label>
                  <Input value={editing.role_title ?? ""} onChange={(e) => setEditing({ ...editing, role_title: e.target.value })} />
                </div>
                <div className="space-y-1.5"><Label>Status</Label>
                  <Select value={editing.status ?? "applied"} onValueChange={(v) => setEditing({ ...editing, status: v as AppStatus })}>
                    <SelectTrigger><SelectValue /></SelectTrigger>
                    <SelectContent>{STATUSES.map((s) => <SelectItem key={s} value={s} className="capitalize">{s}</SelectItem>)}</SelectContent>
                  </Select>
                </div>
                <div className="space-y-1.5"><Label>Source</Label>
                  <Input value={editing.source ?? ""} onChange={(e) => setEditing({ ...editing, source: e.target.value })} placeholder="LinkedIn, referral…" />
                </div>
                <div className="space-y-1.5"><Label>Applied date</Label>
                  <Input type="date" value={editing.applied_at?.slice(0, 10) ?? ""}
                    onChange={(e) => setEditing({ ...editing, applied_at: e.target.value ? new Date(e.target.value).toISOString() : null })} />
                </div>
                <div className="space-y-1.5"><Label>Resume version</Label>
                  <Select value={editing.resume_version_id ?? "none"} onValueChange={(v) => setEditing({ ...editing, resume_version_id: v === "none" ? null : v })}>
                ...
                <div className="space-y-1.5"><Label>Application Tier</Label>
                  <Select value={editing.tier ?? "none"} onValueChange={(v) => setEditing({ ...editing, tier: v === "none" ? null : v })}>
                    <SelectTrigger><SelectValue placeholder="—" /></SelectTrigger>
                    <SelectContent>
                      <SelectItem value="none">None</SelectItem>
                ...
                <div className="space-y-1.5"><Label>Match Score (0-100)</Label>
                  <Input type="number" min={0} max={100} value={editing.match_score ?? ""}
                    onChange={(e) => setEditing({ ...editing, match_score: e.target.value ? Number(e.target.value) : null })} />
                </div>
                <div className="space-y-1.5 flex items-center justify-between border rounded-md p-3 col-span-2 bg-muted/20">
                  <div>
                    ...
                    <div className="text-xs text-muted-foreground">Indicates if tailored resume & QA package are ready</div>
                  </div>
                  <input type="checkbox" className="h-4 w-4 rounded border-gray-300 accent-primary" checked={editing.package_generated ?? false}
                    onChange={(e) => setEditing({ ...editing, package_generated: e.target.checked })} />
                </div>
              </div>
            ...
            <DialogFooter>
              ...
              <Button disabled={save.isPending || !editing.company_name || !editing.role_title}
...
function ApplicationQAPanel({ applicationId, companyName }: { applicationId: string; companyName: string }) {
  ...
  if (answers.length === 0) {
    return (
      <div className="p-4 text-sm text-muted-foreground bg-muted/30 border-t">
        No Q&A generated yet. Click the <Wand2 className="inline h-3 w-3 text-purple-500" /> button to generate an application package.
      </div>
    );
  }
        ...
        {answers.map((qa: any, i: number) => (
          <Card key={qa.id ?? i} className="bg-background/50">
            <CardHeader className="p-3 pb-1">
              <CardTitle className="text-xs font-semibold text-muted-foreground">Q{i + 1}: {qa.question}</CardTitle>
            </CardHeader>
