...
function Landing() {
  return (
    <div className="min-h-screen bg-background text-foreground">
      <a
        href="#main-content"
        className="sr-only focus:not-sr-only focus:fixed focus:top-4 focus:left-4 focus:z-50 focus:px-4 focus:py-2 focus:rounded-md focus:bg-background focus:text-foreground focus:outline-none focus:ring-2 focus:ring-ring"
      >
      ...
      <main id="main-content">
        <section className="mx-auto max-w-5xl px-6 py-24 text-center">
          ...
          <p className="mt-6 text-lg text-muted-foreground max-w-2xl mx-auto">
            VALTREXA-V2 is the workspace where software engineers manage resumes, jobs, recruiter
            relationships, outreach, and analytics without losing track of a single thread.
          </p>
      ...
      <footer className="border-t border-border py-8 text-center text-sm text-muted-foreground">
        © {new Date().getFullYear()} VALTREXA-V2
      </footer>
    </div>
  );
}
