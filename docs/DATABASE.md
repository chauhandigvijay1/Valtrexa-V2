...
- **Migrations:** 19 individual numbered migration files in `supabase/migrations/`
...
| `job_alerts` | *(future)* Planned alert configuration (see `alert_preferences`) |
...
| `events` | *(via `workflow_events`)* Internal workflow event bus entries |
...
| `n8n_webhook_subscriptions` | Registered n8n webhooks per event type |
...
- `current_schema.sql` is a **concatenated reference snapshot** of all 19 migrations — never apply this file directly
...
| `idx_n8n_webhook_subscriptions_user` | `n8n_webhook_subscriptions` | `user_id` |
