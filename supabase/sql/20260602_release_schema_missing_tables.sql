-- Career Compass Pro: release schema fix (missing tables)
-- Paste this entire file into the Supabase SQL Editor and run.
-- This migration is intentionally idempotent (CREATE TABLE IF NOT EXISTS + DROP IF EXISTS).
--
-- Target tables:
-- - resume_parses
-- - resume_analyses
...
-- - loom_scripts
--
-- Source: supabase/migrations/20260531030000_pre_n8n_release_final.sql
...
DROP POLICY IF EXISTS "workflow_events owner select" ON public.workflow_events;
CREATE POLICY "workflow_events owner select" ON public.workflow_events FOR SELECT TO authenticated USING (user_id = auth.uid());
...
CREATE INDEX IF NOT EXISTS idx_notification_queue_user_status ON public.notification_queue(user_id, status, next_attempt_at);
