    ...
    const response = await Promise.race([
      ...
      new Promise<never>((_, reject) =>
        setTimeout(
          () =>
            reject(
              new Error(`invokeRoute timeout after ${ROUTE_TIMEOUT / 1000}s for ${normalizedPath}`),
            ),
          ROUTE_TIMEOUT,
        ),
      ),
...
async function main() {
  ...
  try {
    ...
    const { data: alertPrefs, error: alertPrefsError } = await admin
      .from("alert_preferences")
      .select("id")
      .eq("user_id", userId);
    record(
      "Alert preference trigger",
      !alertPrefsError && (alertPrefs?.length ?? 0) === 1,
      alertPrefsError?.message ?? `rows=${alertPrefs?.length ?? 0}`,
    );
    ...
    record(
      ...
      webhookSub.ok,
      webhookSub.ok ? "Subscription created." : webhookSub.raw,
    );

    const { data: webhookRows, error: webhookRowsError } = await admin
      .from("n8n_webhook_subscriptions")
      .select("id")
      .eq("user_id", userId);
    record(
      "Webhook subscription persistence",
      !webhookRowsError && (webhookRows?.length ?? 0) === 1,
      webhookRowsError?.message ?? `rows=${webhookRows?.length ?? 0}`,
    );
    ...
    const resumeTypes = [
      ...
      {
        label: "DOCX",
        filePath: files.docxPath,
        fileName: "resume.docx",
        contentType: "application/vnd.openxmlformats-officedocument.wordprocessingml.document",
      },
      {
        label: "PDF",
        filePath: files.pdfPath,
        fileName: "resume.pdf",
    ...
    for (const [index, resumeType] of resumeTypes.entries()) {
      ...
      record(
        `Resume upload ${resumeType.label}`,
        processResult.ok,
        processResult.ok ? "Upload + parse succeeded." : processResult.raw,
      );
      if (processResult.ok) {
    ...
    const { data: parses, error: parseError } = await admin
      .from("resume_parses" as any)
      .select("*")
      .eq("user_id", userId);
    record(
      "Resume parser persistence",
      !parseError && (parses?.length ?? 0) >= 3,
      parseError?.message ?? `rows=${parses?.length ?? 0}`,
    );
    ...
    record(
      ...
      !versionError && (versions?.length ?? 0) >= 3,
      versionError?.message ?? `rows=${versions?.length ?? 0}`,
    );
    ...
    record(
      "Refresh persistence",
      !refreshedResumes.error && (refreshedResumes.data?.length ?? 0) >= 3,
      refreshedResumes.error?.message ?? `rows=${refreshedResumes.data?.length ?? 0}`,
    );
    ...
    record(
      "ATS analysis",
      analyzeResult.ok,
      analyzeResult.ok ? `score=${analyzeResult.data.ats_score}` : analyzeResult.raw,
    );
    ...
    record(
      "ATS persistence",
      !analysisError && (analysisRows?.length ?? 0) >= 2,
      analysisError?.message ?? `rows=${analysisRows?.length ?? 0}`,
    );
    ...
    record(
      ...
      tailorResult.ok,
      tailorResult.ok ? "Tailored resume stored." : tailorResult.raw,
    );

    const { data: tailoredRows, error: tailoredError } = await admin
      .from("tailored_resumes")
      .select("id")
      .eq("user_id", userId);
    record(
      "Tailored resume persistence",
      !tailoredError && (tailoredRows?.length ?? 0) >= 1,
      tailoredError?.message ?? `rows=${tailoredRows?.length ?? 0}`,
    );
    ...
    record(
      "Gating blocks research for normal targets",
      !normalGatingRes.ok && normalGatingRes.status === 403,
      `status=${normalGatingRes.status}`,
    );
    ...
    record(
      "Valid OpenRouter user integration behavior",
      integrationKeyResearch.ok,
      integrationKeyResearch.ok
        ? "User-level OpenRouter config worked."
        : integrationKeyResearch.raw,
    );
    ...
    for (const candidate of importCandidates) {
      const importResult = await invokeRoute("/api/jobs/import", {
        method: "POST",
        token,
        body: { sources: [candidate] },
      });
    ...
    const { data: importRunRows, error: importRunError } = await admin
      .from("job_import_runs")
      .select("id")
      .eq("user_id", userId);
    record(
      "Job import run persistence",
      !importRunError && (importRunRows?.length ?? 0) >= 1,
      importRunError?.message ?? `rows=${importRunRows?.length ?? 0}`,
    );
    ...
    if (!jobId) {
      ...
      if (insertedJob.error || !insertedJob.data)
        throw new Error(insertedJob.error?.message ?? "Failed to create fallback job.");
    ...
    record(
      "Job matching",
      matchResult.ok,
      matchResult.ok ? `score=${matchResult.data.score}` : matchResult.raw,
    );

    const supabaseJob = await admin
      .from("jobs")
      .insert({
        user_id: userId,
    ...
    if (!supabaseJob.error && supabaseJob.data) {
      ...
      record(
        ...
        painResult.ok,
        painResult.ok ? `rows=${painResult.data.painPoints?.length ?? 0}` : painResult.raw,
      );
    }
    ...
    record(
      "Company research",
      integrationKeyResearch.ok,
      integrationKeyResearch.ok ? "Research stored." : integrationKeyResearch.raw,
    );
    ...
    record(
      "Outreach generation",
      outreachResult.ok,
      outreachResult.ok ? "Draft stored." : outreachResult.raw,
    );
    ...
    record(
      ...
      loomResult.ok,
      loomResult.ok ? "Script stored." : loomResult.raw,
    );

    const { data: loomRows, error: loomError } = await admin
      .from("loom_scripts")
      .select("id")
      .eq("user_id", userId);
    record(
      "Loom script persistence",
      !loomError && (loomRows?.length ?? 0) >= 1,
      loomError?.message ?? `rows=${loomRows?.length ?? 0}`,
    );
    ...
    record(
      ...
      analyticsSummary.ok,
      analyticsSummary.ok
        ? `applications=${analyticsSummary.data.applications}`
        : analyticsSummary.raw,
    );

    const dailySummary = await invokeRoute("/api/analytics/daily-summary", {
      method: "POST",
      token,
    });
    record(
      ...
      dailySummary.ok,
      dailySummary.ok ? "Summary stored." : dailySummary.raw,
    );
    ...
    record(
      ...
      eventsList.ok,
      eventsList.ok
        ? `events=${Array.isArray(eventsList.data) ? eventsList.data.length : 0}`
        : eventsList.raw,
    );
    ...
    record(
      ...
      !queueError && (queueRows?.length ?? 0) >= 1,
      queueError?.message ?? `rows=${queueRows?.length ?? 0}`,
    );
    ...
    record(
      ...
      webhookList.ok,
      webhookList.ok
        ? `subscriptions=${Array.isArray(webhookList.data) ? webhookList.data.length : 0}`
        : webhookList.raw,
    );

    const { data: storageProbe } = await admin.storage
      .from("resumes")
      .list(`${userId}/verification`);
    record(
      "Storage persistence",
      Array.isArray(storageProbe) && storageProbe.length >= 3,
      `files=${storageProbe?.length ?? 0}`,
    );
    ...
    record(
      "Fallback AI persistence inactive",
      !aiError && (aiRows?.length ?? 0) === 0,
      aiError?.message ?? `rows=${aiRows?.length ?? 0}`,
    );
  } finally {
    await webhook.close();
