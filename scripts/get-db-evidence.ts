...
async function main() {
  ...
  try {
    ...
    const { data: migrations, error } = await admin
      ...
      .select("version");
    if (!error && migrations) {
      console.log("schema_migrations versions:", migrations.map((m: any) => m.version));
    } else {
    ...
    try {
      ...
      if (error) {
        if (/relation .* does not exist|Could not find the table/i.test(error.message)) {
          ...
          console.log(`${table}: exists=YES, count=error (${error.message}), latest_id=N/A, latest_created_at=N/A`);
        }
      } else {
        const rowCount = count ?? 0;
        const latestId = data?.[0]?.id ?? "N/A";
        const latestCreatedAt = data?.[0]?.created_at ?? "N/A";
        console.log(`${table}: exists=YES, count=${rowCount}, latest_id=${latestId}, latest_created_at=${latestCreatedAt}`);
      }
    } catch (err: any) {
      console.log(`${table}: exists=NO, count=0, latest_id=N/A, latest_created_at=N/A (Exception: ${err.message})`);
    }
  }
}
