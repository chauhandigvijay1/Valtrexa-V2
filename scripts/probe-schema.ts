...
async function main() {
  ...
  const { error: fErr } = await admin.from("followups").insert({ user_id: "00000000-0000-0000-0000-000000000000" } as any);
  console.log("followups insert error (expected):", fErr?.message);
  
  console.log("\n=== INTERVIEW_PREPARATION TABLE COLUMNS ===");
  const { error: ipErr } = await admin.from("interview_preparation").insert({ user_id: "00000000-0000-0000-0000-000000000000" } as any);
  console.log("interview_preparation insert error (expected):", ipErr?.message);

  console.log("\n=== APPLICATION_ANSWERS TABLE COLUMNS ===");
  const { error: aaErr } = await admin.from("application_answers").insert({ user_id: "00000000-0000-0000-0000-000000000000" } as any);
  console.log("application_answers insert error (expected):", aaErr?.message);

  console.log("\n=== COMPANIES TABLE COLUMNS ===");
  const { error: cErr } = await admin.from("companies").insert({ user_id: "00000000-0000-0000-0000-000000000000" } as any);
  console.log("companies insert error (expected):", cErr?.message);

  console.log("\n=== FOLLOW_UPS TABLE COLUMNS (dual table) ===");
  const { error: fuErr } = await admin.from("follow_ups").insert({ user_id: "00000000-0000-0000-0000-000000000000" } as any);
  ...
  const { data: apps } = await admin.from("applications").select("id, user_id, company_name, role_title, status, job_id").limit(5);
  console.log(apps);

  // Get existing interviews
  console.log("\n=== EXISTING INTERVIEWS ===");
  const { data: ivs } = await admin.from("interviews").select("id, user_id, company_name, role_title, status, application_id").limit(5);
  console.log(ivs);

  // Get existing recruiters
  console.log("\n=== EXISTING RECRUITERS ===");
  const { data: recs } = await admin.from("recruiters").select("id, user_id, name, company, email").limit(5);
  console.log(recs);
}
