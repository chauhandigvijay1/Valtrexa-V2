...
export async function getProviderControls(): Promise<ProviderControl[]> {
  const { data, error } = await api().select("*").order("provider");
  if (error) throw error;
  return data ?? [];
}

export async function getProviderControl(provider: ProviderName): Promise<ProviderControl | null> {
  const { data, error } = await api().select("*").eq("provider", provider).single();
  if (error && error.code !== "PGRST116") throw error;
  return data ?? null;
}

export async function isProviderEnabled(provider: ProviderName): Promise<boolean> {
  const control = await getProviderControl(provider);
...
export async function setProviderStatus(
  ...
  const { data, error } = await api().update(updates).eq("provider", provider).select().single();
  if (error) throw error;
...
export async function recordProviderFailure(
  ...
  const { error } = await api().update(updates).eq("provider", provider);
  if (error) throw error;
...
export async function logHealthEvent(event: ProviderHealthEvent): Promise<void> {
...
export async function getHealthLog(provider?: ProviderName, limit = 50): Promise<any[]> {
  let query = healthApi().select("*").order("created_at", { ascending: false }).limit(limit);
...
export async function enableAllProviders(): Promise<number> {
  let count = 0;
  for (const p of PROVIDERS) {
    await setProviderStatus(p, "enabled", "bulk");
    count++;
  }
  return count;
}

export async function disableAllProviders(): Promise<number> {
  let count = 0;
  for (const p of PROVIDERS) {
    await setProviderStatus(p, "disabled", "bulk");
    count++;
  }
  return count;
}
